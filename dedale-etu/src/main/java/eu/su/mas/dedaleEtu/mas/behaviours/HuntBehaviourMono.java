...
public class HuntBehaviourMono extends TickerBehaviour {
    ...
    public HuntBehaviourMono(AbstractDedaleAgent agent, List<String> allAgentNames) {
        super(agent, 600);
        ...
        System.out.println(this.myAgent.getLocalName()+ " initialisé en HuntBehaviourMono");
    }
    ...
    public void onTick() {
        ...
        // L'exploration est terminée → on peut chasser sans condition sur meetingPoint
        processMessages(me);
        ...
        if (cur.getLocationId().equals(target)) {
            ...
            boolean moved = moveToward(me, target);
            if (!moved) {
                failedMoves++;
                if (failedMoves >= MAX_FAILED_MOVES) {
                    System.out.println("[" + myName + "] Abandon de " + myClaimedNode + " après " + failedMoves + " échecs");
                    myClaimedNode = null;
                    failedMoves = 0;
                }
            } else {
 ...
 // Ajouter dans les champs
    ...
    private void followerPatrol(AbstractDedaleAgent me) {
        ...
        if (patrolStuckCount > 5 || agentPositions.get(getLeaderName()) == null) {
            // Bloqué ou leader inconnu → patrouille indépendante
            ...
            // Si toujours pas de cible, on revient à la patrouille indépendante
            if (myTargetNode == null) {
                myTargetNode = pickFarNode(map, cur.getLocationId());
                System.out.println("[" + myName + "] Aucune formation, patrouille indépendante : " + myTargetNode);
            }
        }
    ...
    private boolean moveToward(AbstractDedaleAgent me, String targetId) {
        ...
        // Parcourir tout le chemin pour trouver le premier nœud accessible
        for (String nextNode : path) {
            for (var nodeObs : obs) {
                if (nodeObs.getLeft().getLocationId().equals(nextNode)) {
                    boolean success = me.moveTo(nodeObs.getLeft());
                    if (success) return true;
                    else break; // ce nœud est bloqué, passer au suivant
                }
            }
        }
        return false;
